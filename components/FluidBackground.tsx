...
/* eslint-disable */
//
// Ported as-is from the original hero WebGL fluid simulation script. This is
// intentionally left untyped (raw WebGL + shader source strings) so the
// behaviour matches the supplied HTML/JS exactly. It mounts on the canvas
// passed in via ref and returns a `destroy()` cleanup, which React calls on
// unmount instead of the effect just running forever.
...
function fluidSimulation(canvas: HTMLCanvasElement) {
  ...
  initFramebuffers();
  multipleSplats(34);
  for (let i = 0; i < 8; i++) splatStack.push(10 + parseInt(String(Math.random() * 10)));
  ...
  on(window, "touchend", (e: TouchEvent) => {
    ...
    for (let i = 0; i < touches.length; i++)
      for (let j = 0; j < pointers.length; j++)
        if (touches[i].identifier == (pointers[j] as any).id) (pointers[j] as any).down = false;
  });
...
export default function FluidBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("WebGL fluid background failed to start.", e);
    }
